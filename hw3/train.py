...
label_to_idx = {"B-PER": 0, "I-PER": 1, "B-LOC": 2, "I-LOC": 3, "B-ORG": 4, "I-ORG": 5, "O": 6, "<START>": 7, "<STOP>": 8}
...
print(f"Embedding dimension: {embedding_dim} | Hidden dimension: {hidden_dim}  | Tagset size: {tagset_size}  | Vocab size: {vocab_size}")
...
model = BiLSTM_CRF(vocab_size=vocab_size,
                   ...
                   start_tag=label_to_idx["<START>"],
                   stop_tag=label_to_idx["<STOP>"]).to(device)
...
if train_flag:
    ...
    for epoch in range(epochs):
        ...
        for batch in range(num_batches):
            print(f"\rBatch: |", "▇" * int((batch + 1) * (20/num_batches)), " " * int((num_batches - batch - 2) * (20/num_batches)),
                  f"|   {batch + 1}/{num_batches}", end="")
            # zero gradients
            model.zero_grad()
            ...
            # pad sequences
            batch_sentence_indices = pad_sequences([prepare_sequence(sentence, word_to_idx) for sentence in batch_sentences])
            batch_label_indices = pad_sequences([prepare_sequence(label, label_to_idx) for label in batch_labels])

            # forward pass
            total_loss = model.loss(batch_sentence_indices, batch_label_indices)
        ...
        print(f"  Epoch: |", "▇" * int((epoch + 1) * (20/epochs)), " " * int((epochs - epoch - 2) * (20/epochs)), f"|{epoch + 1}/{epochs}",
              f" Loss: {lossv[epoch]}")
    ...
    plt.savefig("out/loss_curve_v1-15000-e200_h300-batch250-adam0.001.png")

    # save model
    torch.save(model.state_dict(), "model/BiLSTM_CRF_v1.pth")
    print("Model Saved to model/BiLSTM_CRF_v1.pth")
else:
    print("Loading model...")
    model.load_state_dict(torch.load("model/BiLSTM_CRF_v1.pth"))
...
for i, (sentence, label) in enumerate(zip(test_sentences, test_labels)):
    sentence_idx = prepare_sequence(sentence, test_word_to_idx)
    label_idx = prepare_sequence(label, label_to_idx).tolist()
    y_true.append([idx_to_label[idx] for idx in label_idx])
    pred_score, pred_tag_seq_idx = model(sentence_idx)
    pred_tag_seq_idx = pred_tag_seq_idx.squeeze(1).tolist()
    pred_tag_seq = [idx_to_label[idx] for idx in pred_tag_seq_idx]
    # print(f"pred_score: {pred_score}\n pred_tag_seq: {pred_tag_seq}")
    y_pred.append(pred_tag_seq)
    print(f"\r|", "▇" * int((i + 1) * (20/len(test_sentences))), " " * int((len(test_sentences) - i - 2) * (20/len(test_sentences))), f"|{i + 1}/{len(test_sentences)}", end="")
# compute accuracy, recall, f1
accuracy = accuracy_score(y_true, y_pred)
recall = recall_score(y_true, y_pred)
f1 = f1_score(y_true, y_pred)
with open("out/eval_v1.txt", "w", encoding="utf-8") as f:
